...
class ProcessTestCase(unittest.TestCase):
    ...
    def test_get_number_of_cores_returns_number(self):
        self.assertTrue(type(self.process_cores) is int or get_process_core_usage(self.process_cores) in ["AccessDenied",
                                                                                                      "Unavailable"])

    def test_get_cpu_usage_returns_number_or_na_or_unavailable(self):
        self.assertTrue(type(self.process_cpu_usage) is float or self.process_cpu_usage in ["AccessDenied",
                                                                                            "Unavailable"])
    ...
    def test_get_process_memory_usage_returns_number(self):
        self.assertTrue(type(self.process_memory_usage) is int or self.process_memory_usage == "AccessDenied")

    def test_get_process_memory_usage_returns_value_between_0_and_68719476736(self):
        self.assertTrue(self.process_memory_usage in range(68719476736))
